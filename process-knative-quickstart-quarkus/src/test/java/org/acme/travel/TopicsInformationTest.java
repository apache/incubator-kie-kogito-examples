...
public class TopicsInformationTest {
    ...
    void verifyTopicsInformation() {
        ...
        expectedIncomingTopic
                .setEventsMeta(Collections.singletonList(new CloudEventMeta("travellers", "", EventKind.CONSUMED)));
        final Topic expectedOutgoingTopic = new Topic("kogito_outgoing_stream", ChannelType.OUTGOING);
        expectedOutgoingTopic.setEventsMeta(Collections.singletonList(
                new CloudEventMeta("process.travelers.processedtravellers", "/process/travelers", EventKind.PRODUCED)));
