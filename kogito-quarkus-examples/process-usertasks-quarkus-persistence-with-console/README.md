...
The example provides instruction to see it deployment with different kind of persistence: postgresql, 
...
- Kogito Task Console
- Keycloak
-(PgAdmin in case of postgresql deployment)
...
Different configuration are provided to try different kinds of persistence:
...
```sh
./target/./target/process-usertasks-quarkus-persistence-with-console-runner
```

### Run Example with MongoDB

#### Compile Hiring example with profile mongodb

First thing is to compile the example with the mongodb profile executing:
...
1. Open a Terminal
2. Go to docker-compose folder
3. Run the ```startServices.sh``` script with mongo argument
...
> **_NOTE:_**  This step requires the project to be compiled, please consider running a ```mvn clean install -Pmongodb``` command on the project root before running the ```startServices.sh mongodb``` script for the first time or any time you modify the project.

Once started you can simply stop all services by executing the ```docker-compose -f docker-compose-mongodb.yml stop```.

All created containers can be removed by executing the ```docker-compose -f docker-compose-mongodb.yml rm```.

#### Run the Hiring example with MongoDB
...
```bash
mvn clean package quarkus:dev -Pmongodb
```
...
```sh
mvn clean package -Pnative -Pmongodb
```
