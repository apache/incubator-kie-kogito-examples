...
public class Application {
    ...
    public void init() throws Exception {
        try (AdminClient client = AdminClient.create(kafkaConfig)) {
            ...
            List<NewTopic> newTopics = asList("kogito.process","kogito.process.dealreviews", "kogito.process.deals").stream().filter(t -> !topics.contains(t)).map(t -> new NewTopic(t, 1, (short) 1)).collect(toList());
