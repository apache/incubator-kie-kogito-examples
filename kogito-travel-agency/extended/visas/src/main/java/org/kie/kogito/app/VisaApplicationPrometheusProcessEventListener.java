...
public class VisaApplicationPrometheusProcessEventListener extends PrometheusProcessEventListener {
    ...
    private static Counter getNumberOfVisaApplicationsApprovedCounter(String appId, String country, String duration,
        ...
        return Counter
                ...
                .tags(Arrays.asList(Tag.of("app_id", appId), Tag.of("country", country), Tag.of("duration", duration),
                        Tag.of("nationality", nationality)))
        ...
        if (processInstance.getProcessId().equals("visaApplications")) {
            ...
            if (application.isApproved()) {
                getNumberOfVisaApplicationsApprovedCounter(identifier, safeValue(application.getCountry()),
                        String.valueOf(application.getDuration()), safeValue(application.getNationality()))
                                .increment();
            } else {
                getNumberOfVisaApplicationsRejected(identifier, safeValue(application.getCountry()),
                        String.valueOf(application.getDuration()), safeValue(application.getNationality()))
                                .increment();
            }
        }
